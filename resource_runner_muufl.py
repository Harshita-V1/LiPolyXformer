...
Merged runner:
 - in-process exec like %run (preserves inline figures)
 - subprocess fallback
 - GPU metrics: nvidia-smi -> pynvml -> torch.cuda
 - robust summarizer and CSV logging
"""
...
def _ensure_summary_header(path):
    ...
    if not os.path.exists(path):
        with open(path, "w", newline="") as f:
            f.write("# Units: time_sec = seconds, avg_gpu_power_W = Watts, avg_gpu_mem_MiB = MiB, avg_RAM_MiB = MiB, energy_J = Joules\n")
            csv.writer(f).writerow(["timestamp","command","time_sec","avg_gpu_power_W","avg_gpu_mem_MiB","avg_RAM_MiB","energy_J","samples"])
...
def run_with_monitor(command_str,
    ...
    if summary_csv:
        ...
        with open(summary_csv, "a", newline="") as f:
            w = csv.writer(f)
            if write_header:
                w.writerow(["timestamp","command","time_sec","avg_gpu_power_W","avg_gpu_mem_MiB","avg_RAM_MiB","energy_J","samples"])
            w.writerow([
